...
read -r -a FASTRANGE_MODES <<< "${MPHF_BENCH_FASTRANGE_MODES:-low32}"
...
echo "==> fixed_gammas=${GAMMAS[*]} auto_gamma_bases=${AUTO_GAMMA_BASES[*]} auto_shape=fixed-pow10" | tee -a "$ERR"
    ...
    if [[ "$RUN_AUTO" == "1" ]]; then
      for n in "${NS[@]}"; do
        for gamma_base in "${AUTO_GAMMA_BASES[@]}"; do
          echo "==> auto n=${n} gamma_base=${gamma_base} auto_shape=pow10 seed_index=${seed_index} key_mode=${key_mode}" | tee -a "$ERR"
          for fastrange_mode in "${FASTRANGE_MODES[@]}"; do
            emit_runner_csv "$BETTER_BIN" \
