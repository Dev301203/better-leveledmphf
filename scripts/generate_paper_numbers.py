...
BITS_CSV = BENCH / "bits_per_key.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--bits-csv", type=Path, default=BITS_CSV)
...
def main() -> None:
    ...
    bits_rows = load_rows(args.bits_csv)
    ...
    bits_exact = [
        ...
        if row["phase"] == "space_bits_per_key" and row["impl"] in {"better-mphf", "bbhash-cpp"}
    ]
    bits_cell = collapse_trials(bits_exact, ["impl", "n", "gamma"])
    bits_map = {
        (str(row["impl"]), int(str(row["n"])), float(str(row["gamma"]))): float(row["ms"])
        for row in bits_cell
    }
    bits_table_ns = [65_536, 262_144, 1_048_576, 2_097_152]
    bits_table_n_suffixes = ["One", "Two", "Three", "Four"]
    bits_table_gammas = sorted({float(str(row["gamma"])) for row in bits_cell})
    bits_table_gamma_suffixes = ["One", "Two", "Three", "Four", "Five", "Six", "Seven", "Eight", "Nine"]

    for suffix, n_value in zip(bits_table_n_suffixes, bits_table_ns):
        add(f"PaperBitsTableN{suffix}", n_value, fmt_int(n_value))

    for gamma_suffix, gamma_value in zip(bits_table_gamma_suffixes, bits_table_gammas):
        add(f"PaperBitsGamma{gamma_suffix}Label", gamma_value, fmt_float(gamma_value, 2))
        for n_suffix, n_value in zip(bits_table_n_suffixes, bits_table_ns):
            add(
                f"PaperBitsGamma{gamma_suffix}BetterN{n_suffix}",
                bits_map[("better-mphf", n_value, gamma_value)],
                fmt_float(bits_map[("better-mphf", n_value, gamma_value)], 4),
            )
            add(
                f"PaperBitsGamma{gamma_suffix}CppN{n_suffix}",
                bits_map[("bbhash-cpp", n_value, gamma_value)],
                fmt_float(bits_map[("bbhash-cpp", n_value, gamma_value)], 4),
            )
    ...
    tex_lines = [
        ...
        f"% Sources: {args.fixed_csv.name}, {args.auto_csv.name}, {args.keymode_csv.name}, {args.fastrange_csv.name}, {args.bits_csv.name}",
    ]
    ...
    payload = {
        ...
        "sources": {
            ...
            "keymode_csv": str(args.keymode_csv),
            "fastrange_csv": str(args.fastrange_csv),
            "bits_csv": str(args.bits_csv),
        },
        "display": display,
        "raw": raw,
    }
